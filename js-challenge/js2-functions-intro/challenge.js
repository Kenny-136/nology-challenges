 ...
 */
export const celsiusToFahrenheit = (tempInCelsius) => `${tempInCelsius} degrees celsius is ${(tempInCelsius * 1.8) + 32} degrees fahrenheit`
 ...
 */
export const calculateLifetimeSupply = (snickersPerDay, age, maxAge) => (maxAge -age) * 365 * snickersPerDay;
 ...
 */
export const calculateAreaOfCircle = (radius) => Number((Math.PI * (radius ** 2)).toFixed(2))
...
export const getStudentSummary = (score, name) => {
  let validScore = 0
  let invalid = `My apologies ${name}, there's been an error in processing your grade.`
  if (typeof score === "number") {
    if (score <= 100) {
      if (score >= 0) {
        validScore = score
